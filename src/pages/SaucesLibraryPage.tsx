...
const CATEGORY_OPTIONS: { id: SauceCategory; label: string; icon: typeof Sparkles }[] = [
  ...
  { id: 'Sauce',     label: 'Sauces',      icon: Droplets },
  { id: 'Dip',       label: 'Dips',        icon: Salad },
  { id: 'Condiment', label: 'Condiments',  icon: Flame },
  { id: 'Dressing',  label: 'Dressings',   icon: Filter },
  { id: 'Marinade',  label: 'Marinades',   icon: ChefHat },
];
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-5">
        ...
        <div className="mt-auto flex items-center gap-3 pt-2 text-xs tracking-tight text-ink-300">
          <span>{sauce.readyInMinutes} min</span>
          <span className="h-1 w-1 rounded-full bg-ink-200" />
          <span>{sauce.servings} servings</span>
        </div>
      </div>
    </Link>
  );
      ...
      <section className="relative overflow-hidden bg-ink-900 text-cream-50">
        <div className="container-wide py-16 md:py-24">
          ...
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-cream-100/70 md:text-lg">
            Fifty four foundational sauces from around the world. The five French mothers, the
            Italian classics, the Mexican salsas, the Arab and Levantine staples, and the Asian
            building blocks. Every one with full ingredients, method, and a verified photo.
          </p>
          <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6">
            {[
              { value: `${SAUCES.length}`, label: 'Sauces' },
              { value: '9', label: 'Cuisines' },
              { value: '5', label: 'Categories' },
              { value: '100%', label: 'No alcohol' },
            ].map((s) => (
      ...
      <section className="container-wide py-16 md:py-20">
        <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-[clamp(1.75rem,3vw,2.5rem)] font-semibold leading-tight tracking-tighter">
              The sauce shelf, sorted.
            </h2>
            <p className="mt-2 max-w-xl text-sm tracking-tight text-ink-500 md:text-base">
              Filter by category, type a word, or scroll until something catches the eye.
            </p>
          </div>
          <div className="relative md:w-80">
            ...
            <input
              ...
              placeholder="Search sauce or ingredient"
        ...
        {filtered.length === 0 ? (
          <div className="rounded-3xl border border-ink-100 bg-cream-50 p-12 text-center">
            <p className="text-base text-ink-600">No sauces match those filters.</p>
            <button
              type="button"
              onClick={() => {
                setCategory('all');
